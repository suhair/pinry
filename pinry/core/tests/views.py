...
from .helpers import UserFactory, PinFactory
from ...core.models import Image


__all__ = ['CreateImageTest', 'PinDetailTest']
...
class CreateImageTest(TestCase):
    ...
    def test_get_xml_http_request(self):
        with self.assertRaises(TemplateDoesNotExist):
            self.client.get(reverse('core:create-image'),
                            HTTP_X_REQUESTED_WITH='XMLHttpRequest')

    def test_post(self):
        with open(settings.SITE_ROOT + 'logo.png', mode='rb') as image:
            response = self.client.post(reverse('core:create-image'),
                                        {'qqfile': image})
...
class PinDetailTest(TestCase):
    ...
    def test_get(self):
        response = self.client.get('/pins/{}/'.format(self.pin.pk))
        self.assertEqual(response.status_code, 200)

    def test_get_404(self):
        response = self.client.get('/pins/2/')
